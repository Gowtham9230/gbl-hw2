...
namespace OpenAi
{
    ...
    {
        public Text correctScore, incorrectScore;
        private int correctValue = 0, incorrectValue = 0;
        private bool isPrefab = false;
        public Button[] answerButtons;
        public Button generateButton;
        public TextMeshProUGUI questionText;
        public string[] questions = { "Lion", "Tiger", "Cat", "Mouse", "Cow", "Snake", "Zebra"};
        ...
        public string GenerateQuestion()
        {
            ...
            correctOptionIndex = UnityEngine.Random.Range(0, answerButtons.Length);
            answerButtons[correctOptionIndex].GetComponentInChildren<Text>().text = currentQuestion;

            for (int i = 0; i < answerButtons.Length; i++)
            {
                if (i != correctOptionIndex)
                {
                    int rand = UnityEngine.Random.Range(0, questions.Length);
                    if(questions[rand] != currentQuestion)
                    {
                        answerButtons[i].GetComponentInChildren<Text>().text = questions[rand];
                    }
                }
            }
        ...
        public void OnOptionButtonClicked(Button button)
        {
            ...
            if (button.GetComponentInChildren<Text>().text == currentQuestion)
            {
                correctValue++;
                correctScore.text = correctValue.ToString();
                Debug.Log("Correct");
            }
            else
            {
                incorrectValue++;
                incorrectScore.text = incorrectValue.ToString();
                Debug.Log("Incorrect!");
            }
        }
        ...
        public async void ReplaceImage(Callback callback = null)
        {
            if(prompt == "")
            {
                prompt = "Picture of " + GenerateQuestion();
            }
